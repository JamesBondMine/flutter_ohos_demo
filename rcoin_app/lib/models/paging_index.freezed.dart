...
abstract class $PagingIndexCopyWith<T, $Res> {
  ...
  @useResult
  $Res call({int total, int pageSize, int page, List<T> list});
}
...
class _$PagingIndexImpl<T> implements _PagingIndex<T> {
  ...
  @override
  String toString() {
    return 'PagingIndex<$T>(total: $total, pageSize: $pageSize, page: $page, list: $list)';
  }
  ...
  @JsonKey(ignore: true)
  @override
  int get hashCode => Object.hash(runtimeType, total, pageSize, page,
...
abstract class _PagingIndex<T> implements PagingIndex<T> {
  const factory _PagingIndex(
      {final int total,
      final int pageSize,
      final int page,
      final List<T> list}) = _$PagingIndexImpl<T>;
